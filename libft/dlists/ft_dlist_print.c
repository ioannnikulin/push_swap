...
/*   Updated: 2024/03/09 21:22:45 by inikulin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	node_and_check(t_dlist *cur, int debug_lvl, void (*p)(void *, int))
{
	int	ret;
	...
	if ((debug_lvl & LIS_INFO) > 0)
		ft_printf(" lisl%i, fl%i, lisn %p ", cur->lisl, cur->flags, cur->lisn);
	if ((debug_lvl & NEXT) > 0)
...
int	ft_dlist_print(t_dlist *lst, int dbg, char *delim, void (*p)(void *, int))
{
	t_dlist	*cur;
	int		lstlen;
	t_dlist	*err;
